...
- [docs/adr/0013-pinescript-indicator-overlay-recipe.md](adr/0013-pinescript-indicator-overlay-recipe.md) — Pine Script / NL → indicator via CLI skill + `compute_indicator` + `apply_research_overlay` (Option A); static-snapshot trade-off vs. live `apply_indicator` (Option B, deferred); `Element.pane` backward-compat rule; single oscillator sub-pane limit. *Read when:* adding indicator support, touching the Pine Script skill, or considering a live-tick overlay MCP tool.
- [docs/adr/0014-overlay-recipe-recompute-model.md](adr/0014-overlay-recipe-recompute-model.md) — `ResearchOverlay.recipe` spec field + auto-save on apply (one-consent model) + pure `recomputeRecipe` frontend helper; extends ADR-0013. *Read when:* touching `bridgeRoundtrip.ts` apply logic, `recomputeRecipe.ts`, or the IndicatorPanel saved-indicator reuse flow.
